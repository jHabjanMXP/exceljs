...
class XLSX {
  ...
  async write(stream, options) {
    ...
    const logExecutionTime = async (label, fn) => {
      if (options.logExecutionTime) {
        console.time(label); // Start timer
      }
      await this.sleep1ms(); // Add sleep before execution
      await fn(); // Execute the actual function
      if (options.logExecutionTime) {
        console.timeEnd(label); // End and log time
      }
    };

    // Render
    await logExecutionTime("addContentTypes", () => this.addContentTypes(zip, model));
    await logExecutionTime("addOfficeRels", () => this.addOfficeRels(zip, model));
    await logExecutionTime("addWorkbookRels", () => this.addWorkbookRels(zip, model));
    await logExecutionTime("addWorksheets", () => this.addWorksheets(zip, model));
    await logExecutionTime("addSharedStrings", () => this.addSharedStrings(zip, model)); // always after worksheets
    await logExecutionTime("addDrawings", () => this.addDrawings(zip, model));
    await logExecutionTime("addTables", () => this.addTables(zip, model));
    await logExecutionTime("addPivotTables", () => this.addPivotTables(zip, model));
    await logExecutionTime("addThemesAndStyles", () => Promise.all([this.addThemes(zip, model), this.addStyles(zip, model)]));
    await logExecutionTime("addMedia", () => this.addMedia(zip, model));
    await logExecutionTime("addAppAndCore", () => Promise.all([this.addApp(zip, model), this.addCore(zip, model)]));
    await logExecutionTime("addWorkbook", () => this.addWorkbook(zip, model));

    return logExecutionTime("finalize", () => this._finalize(zip));
  }
